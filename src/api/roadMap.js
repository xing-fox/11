...
export function initRoadMap (_history, id, a_canvas_h, b_canvas_h) {
...
//画大圆圈
function drawImage (a_context, jsonarray, table_size, icon_size, roadmaps, b_context, c_context, d_context, b_canvas_h, id) {
  var row_array = []; //列的个数
  var tab_size_small;
    ...
    for (c in roadmaps) {
      for (r in roadmaps[c]) {
        if (roadmaps[c][r] != "0000") {
          ...
          a_context.drawImage(img, obj.x, obj.y, icon_size, icon_size, c * table_size, r * table_size, table_size * 1.05, table_size * 1.05);
        }
      }
    }

    if (!b_context) { return };
    drawSmallRoadMap(b_context, tab_size_small, icon_size, roadmap(hollowArray, 10, id), 0);
