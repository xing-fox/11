...
export function drawCanvas (a_canvas, b_canvas, c_canvas, d_canvas, a_canvas1, b_canvas1, c_canvas1, d_canvas1, desk_id, grid_cols, grid_cols_small) {
  var a_context = a_canvas.getContext("2d");
  var b_context = b_canvas.getContext("2d");
  var c_context = c_canvas.getContext("2d");
  var d_context = d_canvas.getContext("2d");
  var a_context1 = a_canvas1.getContext("2d");
  var b_context1 = b_canvas1.getContext("2d");
  var c_context1 = c_canvas1.getContext("2d");
  var d_context1 = d_canvas1.getContext("2d");
  a_contexts[desk_id] = a_context1;
  b_contexts[desk_id] = b_context1;
  c_contexts[desk_id] = c_context1;
  d_contexts[desk_id] = d_context1;
  var grid_rows = 6;
  ...
  a_context1.clearRect(0, 0, a_canvas_w, a_canvas_h);
  ...
  b_context1.clearRect(0, 0, b_canvas_w, b_canvas_h);
  c_context1.clearRect(0, 0, b_canvas_w, b_canvas_h);
  d_context1.clearRect(0, 0, b_canvas_w, b_canvas_h);
...
function roadmap (roadmap_arr, _LZTX, id) {
  var history
  var LZTX = _LZTX;//列数
  var LZTY = 6;//行数
  var arr = [];
          ...
          if (p != p1) {
            ...
            if (row > 6 && !rightBoo) {
              ...
              if (arr[col][row - 1] == "0000" && !rightBoo) {
                ...
                if (col + (row - currentRow) > arr.length - 1) {
                  arr.shift();
                  arr.push(["0000", "0000", "0000", "0000", "0000", "0000"]);
                  col--
                }
                // console.log("----------", col + (row - currentRow), col, maxCol, arr, id)
                arr[col + (row - currentRow)][currentRow - 1] = history[i] + "0" + currentRow;
                rightBoo = true;
              }
            }
...
export function initRoadMap (_history, id, a_canvas_h, b_canvas_h, yuce_result) {
  ...
  if (history != null || history != undefined) {
    ...
    drawImage(a_contexts[id], jsonarray, table_size, icon_size, roadmap(history, 45, id), b_contexts[id], c_contexts[id], d_contexts[id], id, yuce_result, b_canvas_h);
  }
}

//画大圆圈
function drawImage (a_context, jsonarray, table_size, icon_size, roadmaps, b_context, c_context, d_context, id, yuce_result, b_canvas_h) {
  var row_array = []; //列的个数
  var tab_size_small;
  ...
  img.onload = function () {
    ...
    if (!b_context) { return };
    drawSmallRoadMap(b_context, tab_size_small, icon_size, roadmap(hollowArray, 16, id), 0);
    drawSmallRoadMap(c_context, tab_size_small, icon_size, roadmap(solid_array, 16, id), 1);
    drawSmallRoadMap(d_context, tab_size_small, icon_size, roadmap(slash_array, 16, id), 2);
    // return yuce_result
  }
...
function drawSmallRoadMap (context, table_size, icon_size, roadmap, type) {
  ...
  var LZtype = 0;
  var obj;
  context.clearRect(0, 0, b_canvas_w, b_canvas_h);
  for (c in roadmap) {
    for (r in roadmap[c]) {
      ...
      if (roadmap[c][r] != "0000") {
        ...
        obj = json[LZtype].coor;
        context.drawImage(img, obj.x, obj.y, icon_size, icon_size, c * drawing_size, r * drawing_size, drawing_size * 1.03, drawing_size * 1.03);
      }
    }
  }
}
